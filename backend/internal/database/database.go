...
func seedDefaultData(db *gorm.DB) {
	...
	// Seed default share calculation formula
	...
	// Seed transfer fee rates
	transferSettings := []models.SystemSetting{
		{Key: "capital_gain_tax_rate", Value: "15", Description: "Capital gain tax rate on share transfers (percentage). Applied to transfer value."},
		{Key: "transfer_service_fee_rate", Value: "1", Description: "Service fee rate on share transfers (percentage). Applied to transfer value."},
		{Key: "transfer_stamp_duty_rate", Value: "0.5", Description: "Stamp duty rate on share transfers (percentage). Applied to transfer value."},
		{Key: "transfer_vat_rate", Value: "15", Description: "VAT rate applied on transfer service fee (percentage). Applied to service fee amount."},
	}
